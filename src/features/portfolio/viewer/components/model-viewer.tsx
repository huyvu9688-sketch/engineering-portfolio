 ...
 *
 * The component list is an overlay inside the 3D view, shown/hidden by the
 * top-right toggle button. The overall view width is constrained + centered by
 * the page so background shows on both sides.
 */
export function ModelViewer({ src }: ModelViewerProps) {
  ...
  return (
    <div className="relative h-[75vh] min-h-120 w-full overflow-hidden rounded-lg border border-hairline bg-surface-dark">
      ...
      {/* Component list toggle — the only control that hides/shows the list */}
      <button
        type="button"
        id="toggle-list"
      ...
      {/* Component list panel — overlay inside the 3D view, revealed by the
          engine after a model loads and toggled by the button above. */}
      <div
      ...
      >
        <div className="flex h-full flex-col overflow-hidden rounded-lg border border-hairline-dark bg-surface-dark/90 backdrop-blur">
          <div className="border-b border-hairline-dark px-3 py-2.5">
            <span
