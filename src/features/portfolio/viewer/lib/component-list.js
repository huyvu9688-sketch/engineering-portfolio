...
const SEARCH_ICON =
    '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="absolute left-2.5 top-1/2 -translate-y-1/2 text-on-dark-muted pointer-events-none"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.3-4.3"></path></svg>';
...
export class ComponentList {
    ...
    /** Extract the component hierarchy from the loaded model. */
    extractHierarchy(rootObject) {
        ...
        const traverse = (object, level = 0, parentPath = "", parentIndex = null) => {
            ...
            validChildren.forEach((child) => {
                traverse(child, level + 1, currentPath, currentIndex);
            });
        };
    ...
    /** Render the hierarchy into the panel. */
    display(onComponentClick, onComponentContextMenu, onCollapseToggle) {
        ...
        if (headerElement) {
            headerElement.textContent = `Components (${this.hierarchy.length})`;
        }

        listContainer.innerHTML =
            this.buildSearchBar() +
            '<div class="component-list space-y-0.5 p-1">' +
        ...
        return this.hierarchy
            .map((comp, idx) => {
                ...
                <div class="component-item-wrapper ${isHidden ? "hidden" : ""}" data-index="${idx}">
                    <div class="component-item flex items-center rounded px-2 py-1.5 transition-colors hover:bg-white/5"
                         data-uuid="${comp.uuid}">
                        <div class="flex w-full items-center" style="padding-left: ${indent}px;">
                            ${
                                showArrow
                                    ? `<button class="collapse-arrow mr-1 shrink-0 rounded p-0.5 text-on-dark-muted transition-colors hover:bg-white/10"
                                            data-index="${idx}" onclick="event.stopPropagation();" aria-label="Toggle children">
                                            <svg class="arrow-icon" width="10" height="10" viewBox="0 0 24 24" fill="none"
                            ...
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                                 stroke="currentColor" stroke-width="1.5" class="mr-2 shrink-0 text-on-dark-muted">
                                ${icon}
                            </svg>

                            <span class="component-name flex-1 cursor-pointer truncate font-mono text-[10px] text-on-dark"
                                  title="${comp.name}" data-uuid="${comp.uuid}">
                                ${comp.name}
                            </span>

                            <span class="ml-2 shrink-0 rounded-full bg-white/5 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider text-on-dark-muted">
                                ${comp.type}
                            </span>

                            ${
                                comp.meshCount > 0
                                    ? `<span class="ml-2 shrink-0 font-mono text-[8px] text-on-dark-muted/70">${comp.meshCount}</span>`
                                    : ""
                            }
                        </div>
                    </div>
    ...
    toggleCollapse(index) {
        ...
        const arrow = document.querySelector(
            `.collapse-arrow[data-index="${index}"] .arrow-icon`,
        );
        if (arrow) {
    ...
    /** Dim + strike-through parts that are currently hidden in the scene. */
    updateVisibility() {
        this.hierarchy.forEach((comp, idx) => {
            const wrapper = document.querySelector(
                `.component-item-wrapper[data-index="${idx}"]`,
