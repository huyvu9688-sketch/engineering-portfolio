...
const MENU_CLASS =
    "viewer-context-menu fixed z-[60] min-w-[200px] overflow-hidden rounded-lg border border-hairline-dark bg-surface-dark/95 py-1 font-mono text-[11px] uppercase tracking-widest text-on-dark shadow-xl backdrop-blur";
const ITEM_CLASS =
...
export class ContextMenu {
    ...
    show(x, y, targetObject) {
        ...
        this.menuElement.innerHTML = `
            <div class="${ITEM_CLASS}" data-action="isolate">
                ${iconSvg(ICONS.focus)}<span>Isolate</span>
            </div>
            <div class="${ITEM_CLASS}" data-action="toggle-visibility">
                ${iconSvg(isHidden ? ICONS.eye : ICONS.eyeOff)}<span>${isHidden ? "Show" : "Hide"}</span>
            </div>
            <div class="${SEP_CLASS}"></div>
            <div class="${ITEM_CLASS}" data-action="focus">
                ${iconSvg(ICONS.maximize)}<span>Focus</span>
            </div>
            ${
                this.componentList.hiddenParts.size > 0
                    ? `<div class="${SEP_CLASS}"></div>
                       <div class="${ITEM_CLASS}" data-action="show-all">
                           ${iconSvg(ICONS.eye)}<span>Show All</span>
                       </div>`
                    : ""
            }
        `;
    ...
    togglePartVisibility(part) {
        const isCurrentlyHidden = this.componentList.hiddenParts.has(part.uuid);
